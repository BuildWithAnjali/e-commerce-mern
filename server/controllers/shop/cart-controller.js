...
// Utility to format cart items with populated product data
...
const addToCart = async (req, res) => {
  try {
    ...
    if (!userId || !productId || !Number.isInteger(quantity) || quantity <= 0) {
      return res.status(400).json({
        success: false,
        message: "Invalid input. Ensure userId, productId and positive quantity are provided.",
      });
    }
...
const updateCartItemQty = async (req, res) => {
  try {
    ...
    if (!userId || !productId || !Number.isInteger(quantity) || quantity <= 0) {
      return res.status(400).json({
        success: false,
        message: "Invalid input. Provide valid userId, productId, and positive quantity.",
      });
    }
